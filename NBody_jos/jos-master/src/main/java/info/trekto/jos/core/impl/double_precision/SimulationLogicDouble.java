/* ---------------------------------------------------------------
Práctica 1.
Código fuente : SimulationLogicDouble.java
Grau Informàtica
...
public class SimulationLogicDouble extends Kernel implements SimulationLogic {
    ...
    public SimulationLogicDouble(int numberOfObjects, double secondsPerIteration, int screenWidth, int screenHeight, boolean mergeOnCollision,
                                 double coefficientOfRestitution, int Nthreads) {
    ...
    public void calculateAllNewValues() throws InterruptedException{
        int control_start=0;
        int control_finish=0;
        Thread[] th = new Thread[Nthreads];
        for (int h =0; h < Nthreads; h++) {
            int index_start = (int)((readOnlyPositionX.length / Nthreads) * h) + control_start;
            int index_end = (int)((readOnlyPositionX.length / Nthreads) * (h+1)) + control_finish;

            if (readOnlyPositionX.length % Nthreads > h){
                control_finish++;
                index_end += control_finish;
            }
            int finalIndex_end = index_end;
            th[h] = new Thread(() -> {for (int i = index_start; i < finalIndex_end; i++)
                calculateNewValues(i);});
            th[h].start();

            control_start = control_finish;
        }
        for (int h =0; h < Nthreads; h++) {
        th[h].join();
        }
